    ...
    <div className="h-screen w-screen flex flex-col p-2 md:p-6 gap-2 md:gap-4 bg-slate-950 text-slate-50 overflow-hidden">
      ...
      <header className="flex items-center justify-between flex-shrink-0 px-2 mobile-landscape-hide">
        <div className="flex flex-col">
          <h1 className="text-sm md:text-3xl font-black bg-gradient-to-r from-violet-400 to-fuchsia-400 bg-clip-text text-transparent uppercase tracking-tight">
            Pallion Action Group BINGO!
          </h1>
          <span className="text-[6px] md:text-[10px] text-slate-600 font-bold uppercase tracking-[0.3em]">2026 Community Edition</span>
        </div>
        <div className="bg-slate-900/80 px-2 md:px-3 py-0.5 md:py-1 rounded-full border border-white/10 shadow-xl flex items-baseline gap-1 md:gap-2">
          <span className="text-slate-500 font-bold text-[6px] md:text-[10px] uppercase">Drawn</span>
      ...
      <main className="flex-1 min-h-0 flex flex-col md:flex-row mobile-landscape-row gap-2 md:gap-6 overflow-hidden">
        ...
        {/* LEFT PANEL: Shared space on portrait, fixed width on landscape/desktop */}
        <div className="flex-1 md:flex-none md:w-[40%] mobile-landscape-w-left flex flex-col gap-2 md:gap-4 overflow-hidden">
          <div className="gradient-border rounded-xl md:rounded-[2.5rem] flex-1 flex flex-col p-3 md:p-8 shadow-2xl relative overflow-hidden">
            
            {/* 1. DRAW BUTTON */}
            <button
              onClick={drawNumber}
              disabled={gameState.remainingNumbers.length === 0 || isRolling}
              className={`w-full py-4 md:py-12 rounded-lg md:rounded-[1.5rem] bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white font-black text-2xl md:text-6xl shadow-2xl shadow-violet-500/30 active:scale-95 transition-all disabled:opacity-50 border-t border-white/20 flex-shrink-0 ${!isRolling && gameState.remainingNumbers.length > 0 ? 'animate-pulse' : ''}`}
            >
            ...
            <div className="flex-1 flex flex-col items-center justify-center min-h-0 py-2">
              <div className="text-slate-500 font-bold uppercase tracking-[0.4em] text-[7px] md:text-[10px] mb-2 md:mb-3 mobile-landscape-hide">
                {isRolling ? 'Selecting...' : 'Latest Call'}
              </div>
              
              <div className={`relative flex items-center justify-center w-28 h-28 md:w-64 md:h-64 mobile-landscape-num-size rounded-full border md:border-4 border-violet-500/20 bg-slate-950/40 transition-transform duration-300 ${isAnimate ? 'scale-110' : ''}`}>
                <span className={`text-5xl md:text-[10rem] mobile-landscape-text-xl font-black neon-glow transition-all duration-200 
              ...
              {/* 3. RHYME DISPLAY */}
              <div className="mt-2 md:mt-8 min-h-[1.2rem] md:min-h-[4rem] flex items-center justify-center px-4">
            ...
            {/* 4. CONTROLS */}
            <div className="mt-auto pt-2 md:pt-6 flex gap-2 md:gap-3 flex-shrink-0">
              <button onClick={undoLastDraw} disabled={gameState.drawnNumbers.length === 0 || isRolling} className="flex-1 py-2.5 md:py-5 bg-slate-800/60 hover:bg-slate-800 rounded-lg md:rounded-2xl text-[8px] md:text-[10px] font-black text-slate-400 border border-white/5 active:scale-95 transition-all flex flex-col md:flex-row items-center justify-center gap-0.5 md:gap-2">
                <i className="fas fa-undo"></i> <span>UNDO</span>
              </button>
              <button onClick={resetGame} disabled={isRolling} className="flex-1 py-2.5 md:py-5 bg-slate-800/60 hover:bg-slate-800 rounded-lg md:rounded-2xl text-[8px] md:text-[10px] font-black text-red-400/70 border border-white/5 active:scale-95 transition-all flex flex-col md:flex-row items-center justify-center gap-0.5 md:gap-2">
                <i className="fas fa-trash-alt"></i> <span>RESET</span>
              </button>
              <button onClick={shareApp} className="flex-1 py-2.5 md:py-5 bg-violet-600/20 hover:bg-violet-600/40 rounded-lg md:rounded-2xl text-[8px] md:text-[10px] font-black text-violet-400 border border-violet-500/20 active:scale-95 transition-all flex flex-col md:flex-row items-center justify-center gap-0.5 md:gap-2">
                <i className="fas fa-share-alt"></i> <span>SHARE</span>
              </button>
            </div>
          </div>

          {/* Recent History - Hidden on very small portrait height */}
          <div className="bg-slate-900/40 p-2 md:p-3 rounded-xl md:rounded-2xl border border-white/5 mobile-landscape-hide flex-shrink-0">
            <h3 className="text-[7px] md:text-[10px] text-slate-700 font-black uppercase tracking-widest mb-1">Recent Calls</h3>
            <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
              {gameState.drawnNumbers.slice(1, 10).map((num, i) => (
                <div key={i} className="flex-shrink-0 w-7 h-7 md:w-10 md:h-10 rounded-md bg-slate-800 border border-white/5 flex items-center justify-center font-bold text-[9px] md:text-base text-slate-500">
                  {num}
                </div>
              ))}
              {gameState.drawnNumbers.length < 2 && <div className="text-slate-800 font-bold text-[8px] italic py-1">Ready to play...</div>}
            </div>
          </div>
        </div>

        {/* RIGHT PANEL: Shared space on portrait, flex-1 on landscape/desktop */}
      ...
      <footer className="text-center text-slate-800 text-[7px] md:text-[10px] flex-shrink-0 uppercase tracking-[0.4em] font-bold mobile-landscape-hide py-1">
